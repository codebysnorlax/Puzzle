/**
 * Cloudflare Worker Root Entry Point — _worker.js
 * Advanced Workers KV implementation for real visitor tracking & user profile metadata.
    ...
    if (url.pathname === '/api/stats') {
      ...
      if (request.method === 'POST' || url.searchParams.get('action') === 'visit') {
        ...
        if (kv) {
          ...
          // 1. Increment total/frequent visits
          const currentTotal = parseInt(totalStr || '0', 10);
          totalVisits = currentTotal + 1;

          // 2. Handle user record from Workers KV
          let userData = null;
        ...
        return new Response(JSON.stringify({ total: totalVisits, unique: uniqueVisitors, visitorId }), { headers: corsHeaders });
      }

      // GET request - batch KV reads
      let totalVisits = 0;
      let uniqueVisitors = 0;
      if (kv) {
      ...
      return new Response(JSON.stringify({ total: totalVisits, unique: uniqueVisitors }), { headers: corsHeaders });
    }

    // Default static fetch for Cloudflare asset binding
    if (env && env.ASSETS) {
